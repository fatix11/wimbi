...
**Status:** Accepted (supersedes the original Next.js/TypeScript prototype). **Frontend half superseded 2026-09-02 by ADR-010** (Reflex dropped for Django + HTMX) — the backend half (Django + DRF + PostgreSQL) stands unchanged.
...
**Update (2026-09-03):** the open questions below are now largely resolved and the feature is in build — see [bulk-uploader.md](bulk-uploader.md) for the full requirements, the resolved glossary (`new_variable_mapping.csv`, synthesized to ~45 variables across 8 entities), the Country→Program→Data Type funnel, and the v1.1/v1.2+ phasing. Notably: the upload metadata model is answered by OAF's own existing Google Form field set, and the client-identifier requirement had to become scheme-agnostic once `dims/Programs.csv` showed `CROSS_ID` varies by country (NID/APPSHEET_ID/ACCOUNTNUMBER/none).

**Consequences / open design questions (as originally written, now mostly resolved above):**
...
**Update (2026-09-03) — v1.1 scoped and building, see `_docs/bulk-uploader.md`:** the open questions above are addressed for v1.1 specifically (not necessarily their final answer): the data model is `UploadedDataset`/`UploadedRow` with a JSON-shaped mapping (see `bulk_uploader/models.py`); `gl_client_id` linkage stays deliberately unlinked for v1.1; access control is uploader + Admin/Data Team only for now (no new RBAC dimension yet), since the user is deliberately the sole first user while onboarding additional countries. The glossary question ("where does the canonical variable list live?") is fully resolved — `entities-private/samples/new_variable_mapping.csv` is real and already exists, synthesized down to ~45 variables actually consumed by existing FACT/DIM tables. See also ADR-011 for the broader Wimbi↔ANALYTICS co-evolution model this work is part of.

**Update (2026-09-03, later) — v1.1 complete and live-UAT'd; the glossary got its own product surface, not just an uploader implementation detail:** the funnel's Country/Program (and a new Source system question) now cascade off the real mirrored `DimCountry`/`DimProgram`/`DimSystem` rather than a hardcoded list — the earlier "~45 variables" glossary synthesis grew to the ~99 real fields across 8 entities actually needed once tested against real multi-entity files (Client/Location/People/Sale columns in one sheet), with an explicit entities checklist replacing the original single-guess "Data Type" funnel question. Two capabilities emerged directly from real UAT gaps rather than the original design: **synthetic composite keys** (`parsers.build_synthetic_key`) for uploads with no clean source id, mirroring `BRIDGE_CLIENT_SOURCE_IDS`'s own real fallback to composite matching; and a **Glossary page** (`/glossary/`) — a full review of the variable list turned up a real cross-entity naming gap (`loan_name`→`loan_product_name`, GAP-005 in `upstream-gaps.md`), and rather than leave that review as a one-off artifact, it became a permanent, RBAC-open page reusing `base.html` exactly. The end state: this ADR's original "guide a business user through mapping" purpose is now demonstrably true, not just designed — verified against a real 1,000-row Malawi file reaching a genuine "Saved — rows committed" state.
...
## ADR-010: Frontend — Django + HTMX, not Reflex (supersedes ADR-001's frontend choice)

**Status:** Accepted (supersedes ADR-001's "Frontend: Reflex" decision; the backend half of ADR-001 — Django + DRF + PostgreSQL — is unchanged)

**Context:** ADR-001 chose Reflex specifically because the team is Python-fluent and not TypeScript-fluent, and Reflex compiles Python to a real React app for full SPA polish. Building the actual frontend surfaced a real cost that wasn't visible on paper: Reflex's event handlers run server-side, so calling Django's API from a Reflex app means relaying the session cookie manually across two separate origins/processes (parse `Set-Cookie`, store it in `rx.Cookie`, forward it as a header on every `httpx` call) — real and workable, but genuine recurring complexity for what's fundamentally a cross-origin auth problem, not a UI problem.

This prompted a deliberate reflection, done in three rounds rather than settled on the first pass:
1. Wimbi's actual read/write shape is ~90% read / 10% write — the opposite of Odoo's transaction-heavy shape that partly motivated wanting Reflex's SPA smoothness in the first place. A possibly data-team-concentrated audience is also already comfortable with page-reload-y, table-and-chart tools (Superset-like), not necessarily expecting app-smooth navigation.
2. Superset (Flask backend, React/TS frontend) and DataHub (React/TS SPA over a GraphQL gateway) were both examined as real-world precedent for "read-heavy, data-centric tool." Both chose a full JS/TS SPA — a real data point against a server-rendered approach. But both also have genuinely richer UI surfaces than Wimbi's stated MVP: Superset is a chart/dashboard *builder*; DataHub has pan/zoom lineage graphs and inline metadata editing. Wimbi's stated core feature — search across farmers, then a read-and-scroll profile/journey/sales page ("I open, I search, I get history and every touchpoint") — is a bounded master-detail read pattern, not an editor or a graph canvas. HTMX's own flagship pattern (active search, `hx-trigger="keyup changed delay:300ms"`) is a direct match for the search half; the detail page's stated interactions (view profile, scroll a timeline, view sales) don't yet need persistent client-side state across many interactions without a server round trip.
3. The next roadmap feature (Bulk Data Mapper/Uploader, ADR-004) — column-to-glossary mapping after upload — was used as a forward check on the same question, since it's the next most client-interactive screen. It maps cleanly onto the industry-standard CSV-importer pattern (dropdown-per-column assignment over an already-loaded, bounded dataset), with live validation feedback as plain client-side JS (no server round trip needed at all, since the mapping data is already on the page) and drag-and-drop, if wanted later for polish, as a small CDN library (e.g. SortableJS) rather than a framework change. This reinforced rather than strained the HTMX direction.

Also weighed directly: the user is explicit that JS/TS tolerance is low, not just a Python preference — adopting React as the app's organizing architecture (JSX, component state, a build pipeline, TypeScript throughout) is a materially larger, less partially-adoptable cost than a few small, CDN-loaded, self-contained widgets (HTMX, Chart.js, and later SortableJS) layered onto server-rendered templates.

**Decision:** Drop Reflex. Django serves the UI directly: server-rendered templates + HTMX for interactivity (`hx-boost` for app-like navigation, `hx-get`/`hx-trigger` for live search and lazy-loaded page sections), Tailwind for styling — HTMX and Tailwind both loaded via CDN `<script>` tags, no build step, no Node.js dependency at all. Same process and session as the existing DRF API; the API stays exactly as-is under `/api/` for tests and any future integration, with the new HTML views additive on top of the same underlying functions (`get_or_provision_user`, `login_user`, `get_session_user`, `can_access_farmer`, `scope_farmers`/new `scope_queryset`) — nothing about the backend's RBAC or data-access logic changes because of this decision.

**Alternatives considered:**
| Option | Why it was passed over |
|---|---|
| Keep Reflex | Genuine SPA polish, but the server-side-event-handler/cookie-relay complexity is a real, recurring tax on every API call, for a 90%-read app whose core interactions (search, read a detail page) don't need SPA-grade persistent client state |
| React/TypeScript SPA (Superset/DataHub's own pattern) | The strongest alternative on real-world precedent, but Wimbi's current MVP scope (master-detail read, not an editor or graph canvas) doesn't yet need it, and it's a much larger, less partially-adoptable JS/TS cost given the user's own stated low JS tolerance |
| React/TS SPA for just the search+detail pages, HTMX for the rest (hybrid) | A legitimate middle path, but doesn't meaningfully reduce the JS/TS learning cost (you can't really do "a little bit of React") while adding a second rendering paradigm to maintain — not worth it unless the detail page's interaction needs actually grow past what's scoped today |
| Flask (as a lighter-weight alternative to Django for serving HTML) | Would mean maintaining a second web framework alongside Django's own (which the DRF API still needs) for no real gain — Django serves templates natively |

**Consequences:**
- `CORS_ALLOWED_ORIGINS`/`CORS_ALLOW_CREDENTIALS`/`corsheaders` in `config/settings.py`, added for Reflex's cross-origin case, are now dead weight — left in place since removing them isn't load-bearing either way, but they're not doing anything for a same-origin app.
- Real Django CSRF protection now applies to the login page (`{% csrf_token %}` / htmx's `hx-headers` CSRF pattern) — a genuine improvement over the JSON API's `@csrf_exempt`, which was always a deliberate concession for the cross-origin case this decision removes.
- New `accounts/rbac.py:scope_queryset(user, queryset)` — a DB-level RBAC filter — is needed for dashboard aggregates given real row counts (1.3M farmers, 1.5M journey events, 5M sales lines). ~~`scope_farmers`'s materialize-to-a-Python-list approach stays fine for search's small result sets~~ — **wrong, corrected 2026-09-02**: search's result set isn't reliably small at all (see below); `scope_farmers` remains a valid utility (tested, kept) but search now uses `scope_queryset` + a DB-level `LIMIT` instead.

**Lived, not just projected (2026-09-02):** a real, live-browser search for `q=g` against the real 1.3M-farmer table matched a huge fraction of names; `scope_farmers` materialized the entire unbounded match set into a Python list, and rendering it produced a **113MB response** that broke the connection and crashed the dev server process outright (`BrokenPipeError`, process exit). Same class of at-scale problem as ADR-003's CSV-loader `MemoryError` incident, this time in a request path a real user hits directly rather than a one-off batch job — worse in that sense, not better. Fixed in `farmers/views.py`: both the JSON and HTML search endpoints now share `_search_farmers_queryset(user, query)`, which applies `scope_queryset` (DB-level country filter) and a hard `[:50]` slice (Django pushes this down to a SQL `LIMIT`) *before* any results reach Python, plus a minimum 2-character query gate. A single-character query can now never touch more than 50 rows, regardless of how many names it matches. The HTML search page shows "Showing the first 50 matches — refine your search" when the cap is hit, so truncation is visible, not silent.
- **Named tripwire for revisiting this decision** (not hypothetical, a concrete signal): if a future screen needs a real interactive relationship/lineage graph (pan/zoom, node expansion — DataHub-style) or a dashboard *builder* rather than a viewer (KE requirements doc §7, already deferred), that specific screen is a legitimate candidate for a focused JS/TS component — reconsider then, for that screen, not as a wholesale rewrite.
- `reflex`/`reflex-*` packages in `requirements.txt` become unused — left installed for now (no functional cost); removal is a trivial future cleanup once the Reflex frontend code (never built) is confirmed fully gone.

---

## ADR-011: Wimbi and ANALYTICS improve each other in deliberate alternating passes, not sequentially

**Status:** Accepted

**Context:** Designing the Bulk Data Mapper/Uploader (ADR-004, Feature 18) required reading real ANALYTICS artifacts (`entities-private/samples/new_variable_mapping.csv`, `dims/Programs.csv`, `dims/Countries.csv`, `dims/Systems.csv`) that hadn't been consulted before — and doing so immediately surfaced real, concrete gaps: a canonical glossary that exists but had never been referenced from Wimbi's side; per-country client-identifier variation (`CROSS_ID`) that a naive "always require an NID" quality gate would have broken; per-country/program phone-data-quality ratings directly relevant to the already-logged phone-search gap in `upstream-gaps.md`. This mirrors what already happened once before with the frontend build itself — UAT on the built product (`_docs/uat.md`) surfaced GAP-001/002/003 in `upstream-gaps.md`, real ANALYTICS issues that Wimbi's own use surfaced, not something found by auditing ANALYTICS in isolation.

**Decision:** Treat this as the deliberate operating model going forward, not an incidental pattern: build a Wimbi feature → real usage (UAT, or building the next feature against real data) surfaces upstream ANALYTICS gaps → those get fixed at the source (tracked in `_docs/upstream-gaps.md`) → come back to Wimbi and build further, now on better data → repeat. Neither side is meant to be exhaustively finished before the other is touched — the two deliberately co-evolve in alternating passes.

**Rationale:** This is the same principle ADR-007 already applied to a narrower case (never let Wimbi silently diverge from a number Superset would compute differently) — extended here to the wider development process. Building Wimbi features is, in practice, one of the best ways to actually exercise ANALYTICS' real data end-to-end (a UI surfaces a null field or an implausible number in a way a warehouse query alone doesn't), so treating feature work as a *diagnostic* for the data layer — not just a consumer of it — is a genuine efficiency, not a distraction from "real" Wimbi work.

**Consequences:**
- `_docs/upstream-gaps.md` is the literal handoff mechanism between the two repos — every gap found while building or testing Wimbi gets logged there, addressed on the ANALYTICS side independently, and Wimbi revisits the affected feature once the data improves (e.g., GAP-001's Kobo pricing gap, or the phone-search reconsideration this session's `V_REPAYMENT_ANALYSIS` discovery opened up).
- Sequencing decisions (e.g., "build the Bulk Uploader now vs. wait for ANALYTICS improvements first") should default to *not waiting* — build against what's real today, log what's missing, let the next ANALYTICS pass address it.
- This is also why architecture/requirements conversations in this project increasingly start by reading real files in `entities-private/samples/` rather than reasoning from memory or prior summaries of them (as happened here) — every prior instance of doing this (real CSV headers, real schemas, real dims files) has surfaced something a description alone would have missed.
...
**Update (2026-09-03) — the remaining 4 REPORTING views added to the mirror, schema-first:** `V_LOAN_PORTFOLIO` (410,783 rows), `V_PROGRAM_SUMMARY` (62 rows), `V_REPAYMENT_ANALYSIS` (4,321,246 rows), `V_FO_PERFORMANCE` (560 rows) — schemas verified via `DESCRIBE VIEW` + real sample rows (`data/raw/snowflake.sql`), not guessed. New models `LoanPortfolio`/`ProgramSummary`/`RepaymentTransaction`/`FOPerformance` in `analytics_mirror/models.py`, registered in `seed_data.py`'s `MIRROR_MODELS`, empty tables created in the real dev Postgres via a new `ensure_analytics_tables` management command (schema-only, doesn't touch existing data or reseed fixtures). **Data loading mechanism changed for this round**: rather than extending the CSV+chunked-COPY loader (which already had a real memory incident, see below), the user is populating these directly via a Snowflake → Postgres transfer in DBeaver — Django's models define the destination schema first specifically so DBeaver does a data-only load into pre-shaped tables, rather than auto-generating DDL itself (real risk avoided: Snowflake's uppercase column-naming convention could otherwise produce quoted-uppercase Postgres columns our lowercase-expecting ORM wouldn't find). Two real findings worth carrying into feature work: **`V_REPAYMENT_ANALYSIS` has a genuine `REPAYMENT_PHONE` column** (e.g. `+265888354435`) even though `V_CLIENT_REACH` has none at all — Feature 1's "search by phone" story may not be as fully blocked as ADR-006 concluded, worth revisiting once this view is populated. **`V_FO_PERFORMANCE`'s `FO_ID` column is blank even when `FO_KEY`/`FO_NAME` are populated** in the sample — don't rely on it.
...
- **Kobo-sourced tree sales can have a null per-line `total_price_lcy` in `V_SALES_DETAIL` despite real sale activity** (found live-testing farmer `MW-00008737`, 2026-09-02 — `has_sale=True`, 12 real sales lines, but every line's `total_price_lcy`/`unit_price_lcy` is null). Root cause per the user: Kobo datasets often arrive with no unit prices at all, so there's no real per-line total to carry — `revenue_lcy`/`revenue_usd` exist as a prorated stand-in for Superset's own total-revenue reporting, but don't reconcile cleanly to the order total by simple inspection and weren't adopted here without the user's own review. The order-level `total_order_price_lcy` is still populated and identical across every line of the order — Wimbi's sales table does **not** repeat it per line (an earlier attempt did, which misleadingly read as N separate totals); it's summarized once per order (grouped by `source_order_id`) above the table instead, and each line's own Total cell stays an honest "—". The farmer-level `FarmerReach.total_sales_lcy` rollup is also null for this same farmer despite `has_sale=True` — left as "—" rather than locally computed, per ADR-007's rule against re-deriving a number Superset would show differently. Full detail and the user's own follow-up tracking in `_docs/upstream-gaps.md`.
- **No USD/FCY-equivalent total exists on `V_CLIENT_REACH` today** — confirmed via direct schema introspection (all `total_*_lcy` fields are LCY-only, no `total_sales_usd` or similar column). Raised by the user while reviewing the above: showing both LCY and FCY totals on a farmer's profile isn't achievable without a new upstream column — logged as a request in `_docs/upstream-gaps.md`, not built around client-side.
